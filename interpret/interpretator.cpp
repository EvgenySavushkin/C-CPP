...
type_of_lex
Scanner::dlms  [] = { LEX_FIN, LEX_SEMICOLON, LEX_COMMA, LEX_COLON, LEX_ASSIGN, LEX_LPAREN, LEX_RPAREN, LEX_EQ,
...
void Parser::S ()
{
  ...
  else if( c_type == LEX_REPEAT)
  {
     ...
     gl();
     S();
	if ( c_type == LEX_UNTIL)        //L0:S;if(!E)goto L0;
	{
		gl();                      //So E' Po !F
		E();
		eq_bool();
